class Getvect < Formula
  ...
  test do
    ...
    # THIS FIXTURE IS A REAL PNG AND THE FIRST ONE WAS NOT.
    #
    # The original base64 here was written from memory and never executed: zlib
    # rejected it with "incorrect data check", so `brew test` failed at this
    # step from the day it was added. The CLI was verified by hand with a
    # different, valid image, and the two were assumed equivalent. They were not.
    #
    # A test that has never been run is not a test. This one was generated by
    # encoding an 8x8 checkerboard and confirming it traces before it was pasted.
    #
    ...
    (testpath/"in.png").write(
      ("iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAIAAABLbSncAAAAGUlEQVR42mM4ISf3" \
       "69cvTJIBqyiQZBiUOgBp535BlHfoWQAAAABJRU5ErkJggg==").unpack1("m0"),
    )
    ...
    # Help answers on stdout and exits 0.
    assert_match(/usage|Usage|--format/,
                 shell_output("#{bin}/getvect --help"))

    # THE EXIT CODES ARE SYSEXITS AND THE ASSERTION MUST MATCH THE CONTRACT,
    # NOT A GUESS. This asserted 1 until 2026-08-27 and the CLI exits 66 — the
    # expectation was written without running it, which is how the fixture above
    # shipped corrupt too. Both were the same mistake in one test block.
    #
    #   66  EX_NOINPUT    no such file
    #   73  EX_CANTCREAT  output exists and --force was not passed
    shell_output("#{bin}/getvect #{testpath}/nope.png 2>&1", 66)

    # Refusing to clobber is the behaviour a commenter asked about publicly:
    # an agent that overwrites a file it did not know existed is data loss, not
    # a UX preference. out.svg exists by now, so a second run must refuse.
    assert_match(/already exists/,
                 shell_output("#{bin}/getvect #{testpath}/in.png " \
                              "#{testpath}/out.svg 2>&1", 73))
  end
end
